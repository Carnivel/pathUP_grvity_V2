...
const Exams = () => {
    ...
    // Normalize text: lowercase + strip punctuation for fuzzy matching
    ...
    const filteredExams = exams.filter(exam => {
        ...
        if (searchKeywords.length > 0) {
            matchesSearch = searchKeywords.some(keyword =>
                normalize(exam.name).includes(keyword) ||
                normalize(exam.conductedBy).includes(keyword)
            );
        }
                            ...
                            {filteredExams.map(exam => (
                                <div key={exam.id} className="exam-card" style={{ '--accent': exam.color }}>
                                    ...
                                    <div className="exam-card-actions">
                                        ...
                                        <Link href={`/exams/${exam.id}`} className="btn-outline" style={{ color: exam.color, borderColor: exam.color, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
