...
const CoursesInner = () => {
    ...
    // Normalize text: lowercase + strip punctuation (dots, parentheses, hyphens, etc.)
    const normalize = (text) => text.toLowerCase().replace(/[.\-()\/,]+/g, '').replace(/\s+/g, ' ').trim();

    const filteredCourses = courses.filter(course => {
        const matchesStream = activeStream === 'all' || course.stream === activeStream;

        // Smart Search: split by spaces, commas, ampersands, or other common separators
        ...
        if (searchKeywords.length > 0) {
            matchesSearch = searchKeywords.some(keyword =>
                normalize(course.name).includes(keyword) ||
                normalize(course.degree).includes(keyword) ||
                (course.highlights && course.highlights.some(h => normalize(h).includes(keyword)))
            );
        }
                                ...
                                {filteredCourses.map(course => {
                                    ...
                                    return (
                                        <div key={course.id} className={`course-card ${isSelected ? 'selected-for-compare' : ''}`} style={{ '--accent': course.color }}>
                                            ...
                                            <Link href={`/courses/${course.id}`} className="btn-explore" style={{ background: course.color, display: 'flex', textDecoration: 'none' }}>
...
const Courses = () => {
    return (
        <Suspense fallback={<div style={{ padding: '100px', textAlign: 'center' }}>Loading Courses...</div>}>
            <CoursesInner />
        </Suspense>
    );
};
